        ...
        float time,float AirTemperature)
{
 ...
 if(CurrentOn && (LocoOn&4) && eng->Reverse && eng->ThrottlePosition && !SpeedCutoff){
  ...
  if(Velocity>0.0){
   switch(Intercon){
    case 1:
     Force = (150000.0+9000.0*eng->ThrottlePosition)/(pow( (double)Velocity, (double)2.18+(16-eng->ThrottlePosition)*0.06));
     //EngVoltage = 23.4*eng->ThrottlePosition*LineQ;
     HPVoltage=375.0*LineQ;
     //HPVoltage*=1.0+(16-eng->ThrottlePosition)*1.1;
    break;
    case 2:
     Force = (150000.0+10000.0*eng->ThrottlePosition)/(pow((double)Velocity, (double)2.49-eng->ThrottlePosition*0.02));
     //EngVoltage = 375.0+34.1*(eng->ThrottlePosition-16)*LineQ;
     HPVoltage=750.0*LineQ;
    break;
    case 3:
     Force = (250000.0+22000.0*eng->ThrottlePosition)/(pow((double)Velocity, (double)2.16-eng->ThrottlePosition*0.01));
 ...
 if(CurrentOn && (LocoOn&4) && eng->Reverse && eng->var[11]>0.0){
  ...
  if(Velocity>0.0){
   switch(Intercon){
    case 5:
     Force = (1200.0+(eng->var[11]-1.0)*800.0)*(pow((double)Velocity, (double)0.55+(eng->var[11]-1.0)*0.03));
    break;
    case 6:
     Force = (1500.0+(eng->var[11]-16.0)*2200.0)*(pow((double)Velocity, (double)0.68-(eng->var[11]-16.0)*0.02));
    break;
   };
   /*if(Force<0.0)
    Force=0.0;*/
 ...
 if((State>>8)&1){
  if((LocoOn&1)&&cab->Switch(131)){
   ...
   swprintf(buf,L"км%0iпк%0i",int(eng->CurrentMilepost),int(eng->CurrentMilepost*10)%10);
  ...
  if(cab->DisplayState(14)){
   ...
   for(UINT i=0;i<loco->NumSlaves;i++){
    ...
    if(cab->Switch(55+id)){
     ...
     if(CurValue<=999 && CurValue>=-999.0){
      ...
      cab->SetScreenLabel(14,8+id, E_99);
     if(slave->NumEngines>=4)
     ...
     if(CurValue<=999 && CurValue>=-999.0){
      ...
      cab->SetScreenLabel(14,35+id, E_99);
   ...
   //	eng->var[1]=1.0 * cab->Switch(4);
   //	eng->var[2]=1.0 * cab->Switch(4);
        double power = cab->Switch(4);
    eng->var[2] = eng->var[1] = 0.0;
    eng->EngineCurrent[0]= 2* power;
    eng->EngineCurrent[1]= 2* power;
    eng->EngineCurrent[2]= 2* power;
    eng->EngineCurrent[3]= 2* power;
    eng->EngineCurrent[4]= 2* power;
    eng->EngineCurrent[5]= 2* power;

    eng->EngineVoltage[0]= 2* power;
    eng->EngineVoltage[1]= 2* power;
    eng->EngineVoltage[2]= 2* power;
    eng->EngineVoltage[3]= 2* power;
    eng->EngineVoltage[4]= 2* power;
    eng->EngineVoltage[5]= 2* power;
